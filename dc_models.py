...
class SDDCNetwork:
    ...
    def _inference(self, X, eps=1e-6):
        ...
        with tf.variable_scope("output_layer") as scope:
            ...
            if self.distribution.lower() == 'lognormal':
                y = self.probs*(self.loc + self.var / 2) # this is rescaled with inverse_scale_precip = exp(x) 
            else:
                y = self.probs * self.loc
    ...
    def _loss(self, eps=1e-4):
        ...
        with tf.name_scope("prediction"):
            self.prediction = tf.identity(self.inverse_scale_precip(self._prediction_norm),
                                          'prediction')
    ...
    def _build_graph(self):
        ...
        with tf.device(self.device):
            self._prediction_norm = self._inference(self.x_norm)
